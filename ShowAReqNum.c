...
//Creates a temporary file that contains the number of files in "Approved" directory
void create_temp() {
	int id = fork();
	...
	if (id == 0) { //In case this is the child:
		execlp("sh", "sh", "-c", "ls -l Loan/Result/Approved | wc -l > Loan/Result/Approved/temp.txt", NULL);
...
void read_temp(char lines[]) {
	int fd = open_file("Loan/Result/Approved/temp.txt", O_RDONLY, 0);
	char digit;
	int index = 0;
	do {
		read_character(fd, "Loan/Result/Approved/temp.txt", &digit);
		if (digit != ' ') {
			lines[index] = digit;
			index++;
		}
...
void remove_temp() {
	...
	if (id == 0) { //In case this is the child:
		execlp("rm", "rm", "Loan/Result/Approved/temp.txt", NULL);
		//In case couldn't execute:
...
int main(int argc, char* argv[]) {
	//This function gets 1 argument: ShowAReqNum
	if (argc != 1) {
		fprintf(stderr, "You should insert: [ShowAReqNum]\n");
		exit(FAILURE);
	}
	...
	int approved = get_approved(lines); //Number of approved requests
