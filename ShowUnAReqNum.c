...
//Creates a temporary file that contains the number of files in "Unapproved" directory
void create_temp() {
	int id = fork();
	...
	if (id == 0) { //In case this is the child:
		execlp("sh", "sh", "-c", "ls -l Loan/Result/Unapproved | wc -l > Loan/Result/Unapproved/temp.txt", NULL);
...
void read_temp(char lines[]) {
	int fd = open_file("Loan/Result/Unapproved/temp.txt", O_RDONLY, 0);
	char digit;
	int index = 0;
	do {
		read_character(fd, "Loan/Result/Unapproved/temp.txt", &digit);
		if (digit != ' ') {
			lines[index] = digit;
			index++;
		}
...
//Returns the number of unapproved requests
int get_unapproved(char lines[]) {
...
void remove_temp() {
	...
	if (id == 0) { //In case this is the child:
		execlp("rm", "rm", "Loan/Result/Unapproved/temp.txt", NULL);
		//In case couldn't execute:
...
int main(int argc, char* argv[]) {
	//This function gets 1 argument: ShowUnAReqNum
	if (argc != 1) {
		fprintf(stderr, "You should insert: [ShowUnAReqNum]\n");
		exit(FAILURE);
	}
	...
	int unapproved = get_unapproved(lines); //Number of unapproved requests
