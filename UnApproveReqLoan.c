...
//Changes the status from "Waiting!" to "Unapproved" and moves the file to "Unapproved" directory
void change_status(char id[]) {
	char path[PATH_LEN] = "Loan/Request/";
	create_path(path, id);
	int fd = open_file(path, O_RDWR, 0);
	seek_lastln(fd, path);
	write_string(fd, path, "Status: Unapproved");
	close(fd);
	move_file(path, "Loan/Result/Unapproved");
}

int main(int argc, char* argv[]) {
	//This function gets 2 arguments: UnApproveReqLoan, id
	if (argc != 2) {
		fprintf(stderr, "You should insert: [UnApproveReqLoan] [id]\n");
		exit(FAILURE);
	}
